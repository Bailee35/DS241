...
```
```{r}
pop_data <- wb_data("SP.POP.TOTL" , start=2019 , end=2019) %>%
...
```{r}
Covid_TP=left_join(country_data , pop_data , by=("country"))
...
```{r}
ggplot(Covid_TP) + geom_point (aes(x=days_elapsed, y=cumulative_cases, color=country))
```
...
```{r}
Covid_TP2=left_join(country_data , coronavirus_df , by=("country"))
```

```{r}
ggplot(Covid_TP2) + geom_point (aes(x=days_elapsed, y=cumulative_cases, color=country))
```

I am not sure why I got the same graph as I did above with the country_data and pop_data left join, when I used the new coronavirus data***
